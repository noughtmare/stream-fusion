...
-- | A stream step.
--
--   A step either ends a stream, nexts a value, or yields a value
--
...
tail (Stream next0 s0) = Stream next (S1 :!: s0)
  where
    ...
    next (S1 :!: s) = case next0 s of
      ...
      Yield _ s' -> next (S2 :!: s') -- drop the head
...
intersperse sep (Stream next0 s0) = Stream next (s0 :!: Nothing :!: S1)
  where
    ...
    next (_ :!: (Just (L _)) :!: S2) = moduleError "interperse.next" "can't happen"
...
concat (Stream next0 s0) = Stream next (Nothing :!: s0)
  where
    {-# INLINE next #-}
    next (Just (L [])     :!: s) = expose s $ next    (Nothing     :!: s)
    next (Just (L (x:xs)) :!: s) = expose s $ Yield x (Just (L xs) :!: s)

    next (Nothing :!: s) = case next0 s of
      Done        -> Done
      next     s' -> expose s' $ next (Nothing     :!: s')
      Yield xs s' -> expose s' $ next (Just (L xs) :!: s')
-}
...
concatMap f (Stream next s0) = loop_concatMap_to s0
  where
    ...
    loop_concatMap_to !s = case next s of
      Done       -> []
      next    s' -> expose s' $ loop_concatMap_to       s'
...
concatMap f (Stream next0 s0) = Stream next (Nothing :!: s0)
  where
    ...
    next (Nothing :!: s) = case next0 s of
      Done       -> Done
      next    s' -> expose s' $ next (Nothing        :!: s')
      Yield a s' -> expose s' $ next (Just (L (f a)) :!: s')
...
concatMap' nextb f (Stream nexta sa0) = Stream next (sa0 :!: Nothing)
  where
    ...
    next (sa :!: Just sb) = case nextb sb of
      ...
      next    sb' -> next    (sa :!: Just sb')
      Yield b sb' -> Yield b (sa :!: Just sb')

    next (sa :!: Nothing) = case nexta sa of
      Done        -> Done
      next    sa' -> next (sa' :!: Nothing)
      Yield a sa' -> next (sa' :!: Just (f a))
-}
...
drop :: Int -> Stream a -> Stream a
drop n0 (Stream next0 s0) = Stream next (max 0 n0 :!: s0)
  where
    {-# INLINE next #-}
    next (0 :!: s) =
      case next0 s of
        Done -> Done
        Yield x s' -> Yield x (0 :!: s')
    next (n :!: s) =
      case next0 s of
        Done -> Done
        Yield _ s' -> next ((n - 1) :!: s')
{-# INLINE [0] drop #-}
...
zipWith f (Stream next0 sa0) (Stream next1 sb0) = Stream next (sa0 :!: sb0 :!: Nothing)
  where
    ...
    next (sa :!: sb :!: Nothing)     = case next0 sa of
        Done        -> Done
        Yield a sa' -> next (sa' :!: sb :!: Just (L a))
...
zipWith3 f (Stream nexta sa0)
  ...
  where
    ...
    next (sa :!:  sb :!:  sc :!: Nothing) = case nexta sa of
      Done        -> Done
      Yield a sa' -> next (sa' :!: sb :!: sc :!: Just (L a :!: Nothing))

    next (sa' :!: sb :!:  sc :!: Just (L a :!: Nothing)) = case nextb sb of
      Done        -> Done
      Yield b sb' -> next          (sa' :!: sb' :!: sc :!: Just (L a :!: Just (L b)))
...
zipWith4 f (Stream nexta sa0)
  ...
  where
    ...
    next (sa :!:  sb :!:  sc :!: sd :!: Nothing) =
        case nexta sa of
            Done        -> Done
            Yield a sa' -> next (sa' :!: sb :!: sc :!: sd :!: Just (L a :!: Nothing))

    next (sa' :!: sb :!:  sc :!: sd :!: Just (L a :!: Nothing)) =
        case nextb sb of
            Done        -> Done
            Yield b sb' -> next (sa' :!: sb' :!: sc :!: sd :!: Just (L a :!: Just (L b :!: Nothing)))

    next (sa' :!: sb' :!: sc :!: sd :!: Just (L a :!: (Just (L b :!: Nothing)))) =
        case nextc sc of
            Done        -> Done
            Yield c sc' -> next (sa' :!: sb' :!: sc' :!: sd :!: Just (L a :!: (Just (L b :!: Just (L c)))))
...
genericDrop n0 (Stream next0 s0) = Stream next (Just (L n0) :!: s0)
  where
    {-# INLINE next #-}
    next (Just (L 0) :!: s) = next (Nothing :!: s)
    next (Just (L n) :!: s) = case next0 s of
      Done                    -> Done
      Yield _ s' | n > 0      -> next (Just (L (n-1)) :!: s')
...
guard b (Stream next0 s0) = Stream next (S1 :!: s0)
  where
    {-# INLINE next #-}
    next (S1 :!: s) = if b then next (S2 :!: s) else Done
    next (S2 :!: s) = case next0 s of
      Done       -> Done
      Yield x s' -> Yield x (S2 :!: s')
{-# INLINE [0] guard #-}
